...
## 2026-08-27 — `USBPolyphonicMidiController` JSON support classified as unfinished/dormant

**Decision:** Investigate the empty `nw2s::USBPolyphonicMidiController::create(aJsonObject* data)` factory, classify the result, and explicitly **do not** implement or enable polyphonic USB MIDI JSON support as part of the current preservation/modernisation work.

**Investigation outcome:**

- The empty `USBPolyphonicMidiController::create(aJsonObject* data)` body at `gcc/app/src/devices/UsbMidi.cpp:936-938` has been on disk since the original 2015-05-26 JSON work (commit `aa84bda` — `checkpoint getting json working`) and has never been filled in. No implementation has ever existed in the repository history.
- No bundled or preserved SD program JSON under `flash/programs/` or `flash/programs/examples/usbmidi/` uses `"type": "USBPolyphonicMidiController"`. No surviving JSON example, sample, documentation fragment, comment or historical implementation establishes the intended field names or voice-array structure. The exact JSON contract therefore cannot be reconstructed without speculation.
- By analogy with the four working USB MIDI JSON factories (`USBMidiCCController`, `USBMonophonicMidiController`, `USBSplitMonoMidiController`, `USBMidiTriggers` at `gcc/app/src/devices/UsbMidi.cpp:511-518, 582-605, 713-750, 983-1009`), a likely intended shape is inferable: read a single `aftertouch` analog output, construct the controller, iterate a per-voice array of `{ gate, triggerOn, triggerOff, pitch, velocity, pressure }` entries, call `addVoice(...)` for each, and return the object. The precise field names — particularly the wrapper key for the voice array — are not verifiable from the tree.
- The class itself is **unfinished**. `USBPolyphonicMidiController::onNoteOn(uint32_t, uint32_t, uint32_t)` at `gcc/app/src/devices/UsbMidi.cpp:959-981` is entirely commented out, and no working note-off, pressure, aftertouch or pitch-bend handler is supplied. Implementing only the JSON factory would therefore still not produce a working polyphonic USB MIDI controller.
- The dispatch at `gcc/app/src/util/SDFirmware.cpp:452-454` calls `EventManager::registerDevice(...)` on the factory result but does **not** call `EventManager::registerUsbDevice(...)`, unlike every other working USB MIDI controller path in the same dispatch. Implementing only the factory would still leave the controller unwired into the USB host task.
- The empty non-void factory is a latent undefined-behaviour path: per C++98 §6.6.3/2 (unchanged in later standards), reaching the end of a value-returning function without a `return` is undefined. Both GCC 4.8.3 and GCC 6.3.1 emit `no return statement in function returning non-void [-Wreturn-type]` under `-Wall -Wextra -Wreturn-type`; the production Makefile suppresses the warning with `-w`. Under the project's `-Os` and the AAPCS the function most likely returns NULL on this codebase, which on the next `EventManager::loop()` tick would dereference a NULL virtual-call pointer and trigger a Cortex-M3 HardFault. None of the preserved or bundled SD program examples currently takes this branch.

**Reason:** The latent UB path is straightforward to remove with a one-line `return NULL;`, but doing so would still leave the controller non-functional (no note handling, no USB registration) and would silently no-op for any user who authors an SD program selecting this type. Implementing a working JSON factory would require (a) a verified JSON contract, (b) completion of the polyphonic note-handling implementation, (c) the missing `registerUsbDevice(...)` call in the same dispatch arm, (d) a dedicated example configuration, (e) dual-toolchain build verification, and (f) hardware USB-host validation. Each of those is its own reviewed change. AGENTS.md and the previous decision recorded above (the 2026-08-27 "Known outstanding missing-return defects" entry) both call for preservation-first work and forbid speculative new functionality.

**Consequence:** This defect is reclassified from "outstanding missing-return" to **unfinished/dormant feature containing a latent undefined-behaviour path**. **No source change has been made.** The currently validated GCC 6 firmware SHA-256 `be8505d77e295c6fd39e9c01c526748374c56b4dcdcc4414785f3f89e42cbce0` remains the post-investigation firmware. The empty factory body, the commented-out `onNoteOn`, and the missing `registerUsbDevice(...)` call in the dispatch arm all remain on disk unchanged.
