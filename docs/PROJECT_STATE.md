...
- The polyphonic USB MIDI controller JSON support has now been investigated on 2026-08-27 and classified as **unfinished/dormant feature containing a latent undefined-behaviour path** rather than an immediately actionable defect. The empty factory has had no implementation since it was added in commit `aa84bda` in May 2015, no surviving JSON example establishes the intended field names or voice-array structure, the class's `onNoteOn` is commented out, and the dispatch in `SDFirmware.cpp` also fails to call `EventManager::registerUsbDevice(...)` for this type. The full classification, evidence and reasons are recorded in `docs/DECISIONS.md` (2026-08-27 — `USBPolyphonicMidiController` JSON support classified as unfinished/dormant). **No source change was made.** The currently validated GCC 6 firmware SHA-256 `be8505d77e295c6fd39e9c01c526748374c56b4dcdcc4414785f3f89e42cbce0` is unchanged, and the dormant path is not exercised by any preserved or bundled SD program example. Future enablement of polyphonic USB MIDI JSON support would require an explicit JSON contract, completion/review of the polyphonic note-handling implementation, correct USB-device registration in `SDFirmware.cpp`, a dedicated example configuration, dual-toolchain build verification, and hardware USB-host validation, and is **not** part of the current preservation/modernisation scope.

- The USB MIDI arpeggiator JSON support has now been investigated on 2026-08-27 and classified as **unfinished/dormant JSON support for an otherwise substantially functional typed C++ device**. The empty `USBMidiApeggiator::create(aJsonObject* data)` body has had no implementation since it was added in commit `aa84bda` in May 2015; no bundled, preserved or historical SD program uses `"type": "USBMidiApeggiator"`; the typed-factory class is substantially implemented (note-on / note-off, clocked `reset()`, latch, density, octaves, gate, pitch, velocity, trigger, sort order) and was demonstrably used by `bDemoMidiArp.ino` and `gcc/app/src/firmware/midiArpMain.cpp`; and the dispatch in `SDFirmware.cpp` is correctly structured (it requires a valid `clockDevice`, calls both `EventManager::registerUsbDevice(...)` and `clockDevice->registerDevice(...)`, and does not incorrectly register as an ordinary `TimeBasedDevice`). However, no trustworthy historical JSON contract survives in the repository: `controllerMap` is established by the base class, several pin field names are strongly inferable from sibling factories but not proven for this device, the trigger field spelling and the `octaves` parameter semantics (the parameter is actually a `PinAnalogIn`, not a count) are uncertain, no JSON-to-`NoteStackSortOrder` helper exists in the project, and the class's pattern-related and advanced-feature declarations were never completed. The full classification, evidence and reasons are recorded in `docs/DECISIONS.md` (2026-08-27 — `USBMidiApeggiator` JSON support classified as unfinished/dormant). **No source change was made.** The currently validated GCC 6 firmware SHA-256 `be8505d77e295c6fd39e9c01c526748374c56b4dcdcc4414785f3f89e42cbce0` is unchanged, and the dormant JSON path is not exercised by any preserved or bundled SD program example. Implementing JSON support now would require inventing field names and a new `NoteStackSortOrder` JSON bridge, so a future deliberately-designed JSON interface would be **feature/interface development, not restoration**, and is **not** part of the current preservation/modernisation scope. The empty factory remains a latent undefined-behaviour / hard-fault path only if a user explicitly authors an SD program selecting `"USBMidiApeggiator"`.

- Four additional items were observed during the arpeggiator investigation and are recorded here as **separate investigation candidates only** — they are not part of the current preservation/modernisation scope and must not be folded into other unrelated changes. Each requires its own dedicated investigation cycle:

  1. **[RESOLVED 2026-08-27]** The constructor statement `this->pattern = pattern;` at `gcc/app/src/devices/UsbMidi.cpp:1076` was a self-assignment: the `USBMidiApeggiator` constructor had no parameter named `pattern` (it was removed in commit `71247a4`). The statement wrote the just-default-constructed `std::vector<uint32_t> pattern` member to itself. It has now been removed. GCC 4.8.3 `.o` size dropped by 188 bytes; GCC 6.3.1 `.o` size dropped by 174 bytes. The final firmware remained byte-for-byte identical under both toolchains, and validated GCC 6 SHA-256 remains `be8505d77e295c6fd39e9c01c526748374c56b4dcdcc4414785f3f89e42cbce0`. No hardware reflash was required.
  2. Four `USBMidiApeggiator` methods are declared at `gcc/app/src/devices/UsbMidi.h:366-370` but never defined in the tracked tree: `setPattern(std::vector<uint32_t>)`, `setPatternSelector(PinAnalogIn)`, `addTriggerPin(uint32_t, PinDigitalOut)`, and `setClockInput()`. Any code that references them will fail to link.
  3. The members `std::vector<uint32_t> pattern` (constructor-referenced only via the self-assignment above) and `uint32_t patternIndex = 0` (declared but never written or read outside the class declaration) appear to be dead/incomplete feature state. The 16 hardcoded `patterns[]` array added by `71247a4` at `UsbMidi.h:140-158` lives on `USBMidiDevice` and is unused by `USBMidiApeggiator`.
  4. The `AnalogOut* pressure` and `AnalogOut* afterTouch` outputs are constructed and stored by the constructor, but the current `USBMidiApeggiator` implementation does not drive them — `onPressure` / `onAftertouch` are commented out at `UsbMidi.h:377-379`, `onPitchbend` is also commented out, and `reset()` does not output them.
