...
- Four additional items were observed during the arpeggiator investigation and are recorded here as **separate investigation candidates only** — they are not part of the current preservation/modernisation scope and must not be folded into other unrelated changes. Each requires its own dedicated investigation cycle:
  ...
  1. The constructor statement `this->pattern = pattern;` at `gcc/app/src/devices/UsbMidi.cpp:1076` is a self-assignment: the `USBMidiApeggiator` constructor has no parameter named `pattern` (it was removed in commit `71247a4`), so the statement writes the just-default-constructed `std::vector<uint32_t> pattern` member to itself with no observable effect. It is a probable leftover from the earlier 11-parameter constructor signature.
